...
def compress(choose, des_dir, src_dir, file_list, sign):
    ...
    for infile in file_list:
        ...
        os.rename(des_dir + infile, des_dir + infile.replace(sign, ''))
def compress_photo(sign):
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = "min_photos/", "min_photos/"
    ...
    if directory_exists(des_dir):
        ...
        # print file_list
    '''如果已经压缩了，就不再压缩'''
    for filename in tuple(file_list_src):
        # if file_list_des[i] in file_list_src:
            # file_list_src.remove(file_list_des[i])
        if not filename.startswith(sign):
            file_list_src.remove(filename)
    print(file_list_src)
    compress('4', des_dir, src_dir, file_list_src, sign)
...
def handle_photo():
    ...
    for i in range(len(file_list)):
        ...
        if i == 0:  # 处理第一个文件
            ...
            list_info[-1]['arr']['width'].append(w)
            list_info[-1]['arr']['height'].append(h)
        ...
        if file_list:
            ...
            for infile in file_list:
                ...
                if not os.path.isfile(ter_dir + infile): 
                    Graphics(infile=src_dir+infile, outfile=ter_dir + sign + infile).cut_by_ratio()            
        else:
            pass
    else:
...
if __name__ == "__main__":
    sign = '||~0~||' # to mark as already compressed img
    cut_photo(sign)        # 裁剪图片，裁剪成正方形，去中间部分
    compress_photo(sign)   # 压缩图片，并保存到mini_photos文件夹下
